...
### Summary
The objective of this analysis is to look at the variance and standard error of a simulation of numbers drawn at random from the exponential distribution. This distribution has a number of known properties of the theoretical total population, and this analysis will show that the statistical properties of the sample dataset are close to the expected values as predicted by the theoretical population properties.
...
Variable Name | Mathematical Symbol | Description | Calculated As
------------- | ------------------- | ----------- | -------------
lambda | <math display="inline"><mo>&lambda;</mo></math> | rate parameter of the exponential distribution | n/a
...
sigma2 | <math display="inline"><msup><mo>&sigma;</mo><mn>2</mn></msup></math> | population variance | <math display="inline"><msup><mfenced><mfrac bevelled="true"><mrow><mn>1</mn></mrow><mrow><mo>&lambda;</mo></mrow></mfrac></mfenced><mn>2</mn></msup></math>
...
xbar | <math display="inline"><mover><mi>x</mi><mo>&OverBar;</mo></mover></math> | sample mean | <math display="inline"><mfrac><mrow><mn>1</mn></mrow><mrow><mi>n</mi></mrow></mfrac><mrow><munderover><mo>&Sigma;</mo><mrow><mi>i</mi><mo>=</mo><mn>1</mn></mrow><mi>n</mi></munderover><msub><mi>x</mi><mi>i</mi></msub></mrow></math>
s2 | <math display="inline"><msup><mi>s</mi><mn>2</mn></msup></math> | sample variance | <math display="inline"><mfrac><mrow><mn>1</mn></mrow><mrow><mi>n</mi><mo>-</mo><mn>1</mn></mrow></mfrac><mrow><munderover><mo>&Sigma;</mo><mrow><mi>i</mi><mo>=</mo><mn>1</mn></mrow><mi>n</mi></munderover><msup><mfenced><mrow><msub><mi>x</mi><mi>i</mi></msub><mo>-</mo><mover><mi>x</mi><mo>&OverBar;</mo></mover></mrow></mfenced><mn>2</mn></msup></mrow></math>
muxbar | <math display="inline"><msub><mo>&mu;</mo><mover><mi>x</mi><mo>&OverBar;</mo></mover></msub></math> | mean of sample means | <math display="inline"><mfrac><mrow><mn>1</mn></mrow><mrow><mi>s</mi></mrow></mfrac><mrow><munderover><mo>&Sigma;</mo><mrow><mi>i</mi><mo>=</mo><mn>1</mn></mrow><mi>s</mi></munderover><msub><mover><mi>x</mi><mo>&OverBar;</mo></mover><mi>i</mi></msub></mrow><mtext>(where s is the number of simulations)</mtext></math>
s2xbar | <math display="inline"><msubsup><mi>s</mi><mover><mi>x</mi><mo>&OverBar;</mo></mover><mn>2</mn></msubsup></math> | variance of sample means | <math display="inline"><mfrac><mrow><mn>1</mn></mrow><mrow><mi>s</mi><mo>-</mo><mn>1</mn></mrow></mfrac><mrow><munderover><mo>&Sigma;</mo><mrow><mi>i</mi><mo>=</mo><mn>1</mn></mrow><mi>s</mi></munderover><msup><mfenced><mrow><msub><mover><mi>x</mi><mo>&OverBar;</mo></mover><mi>i</mi></msub><mo>-</mo><msub><mo>&mu;</mo><mover><mi>x</mi><mo>&OverBar;</mo></mover></msub></mrow></mfenced><mn>2</mn></msup></mrow><mtext>(where s is the number of simulations)</mtext></math>
sxbar | <math display="inline"><msub><mi>s</mi><mover><mi>x</mi><mo>&OverBar;</mo></mover></msub></math> | standard deviation of sample means | <math display="inline"><msqrt><msubsup><mi>s</mi><mover><mi>x</mi><mo>&OverBar;</mo></mover><mn>2</mn></msubsup></msqrt></math>
...
sem <- sigma / sqrt(n)
```
```{r introOutput, echo=FALSE}
d <- data.frame(
    lambda = lambda, mu = mu, sigma = sigma, sigma2 = sigma2, n = n, sem = sem)
print(matrix(
    d, ncol=1, dimnames=list(colnames(d), c("value"))))

```

### Distribution of Sample
Here I take a random sample of `r n * numsims` variables from the exponential distribution. These same values will be split later into `r numsims` groups of `r n` for multiple sample means to be taken, but here I plot the distribution of all of the sampled values.
```{r expSample, echo = FALSE}
set.seed(20150109)
...
```
Whereas the sample of random exponential values followed the expected exponential distribution, figure 2 shows that these sample means follows a normal distribution. It also shows that the mean of the sample means (`r muxbar`) is very close to the population mean (`r mu`) and that the standard deviation of the sample means (`r sxbar`) is very close to the expected value, the standard error of the mean (`r sem`).

Note that the mean of all samples (xbar = `r xbar`) is identical to the mean of the sample means (muxbar = `r muxbar`).

The standard deviation of sample means (`r sxbar`) is significantly smaller than the population standard deviation, sigma (`r sigma`). This will be because most of the variance of the randomly sampled data is essentially discarded when we take means of those sampled values.