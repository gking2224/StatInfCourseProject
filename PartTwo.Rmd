...
Although there is a small amount of skew in some of the subsets of the data, I will assume that this is not enought to invalidate the use of *t* interval.

```{r explore4}
<<tgExplore4>>
```
Figure 4 shows that the Vitamin C dose has an impact with both of the supplements used, although it is more marked in the Ascorbic Acid subset.

The combination of these plots shows that the impact on tooth length is more reliably observed from the sample data in 0.5mg doses of Ascorbic Acid (shorter tooth length) and 2mg doses of Orange Juice (longer tooth length).

### Confidence Intervals

Mean tooth length for each supplement type will fall between below lower and upper limits, with 95% confidence interval:
```{r ci1}
<<ci1>>
```

Mean tooth length for each dose will fall between below lower and upper limits, with 95% confidence interval:
```{r ci2}
<<ci2>>
```
